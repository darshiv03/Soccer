...
# Determine the project root directory. Adjust if your structure differs.
...
def create_templated_video(input_video_path, text_string, output_path, template_path, video_pos=(100, 1150)):
    try:
        ...
        # Wait until the file size is stable (ensuring all data is flushed)
        stable = False
        prev_size = -1
        while not stable:
            time.sleep(0.1)
        ...
        # Repackage the video with FFmpeg to move the moov atom to the front for streaming (faststart)
        ...
        print("Running FFmpeg to repackage the video for streaming...")
        result = subprocess.run(ffmpeg_cmd, stdout=subprocess.PIPE, stderr=subprocess.PIPE)
        if result.returncode != 0:
            print("FFmpeg error:", result.stderr.decode())
            raise Exception("FFmpeg repackaging failed.")
