...
# Determine the project root directory. This assumes that video_service.py is located in project_root/app/services/
BASE_DIR = os.path.abspath(os.path.join(os.path.dirname(__file__), "..", ".."))

def create_templated_video(input_video_path, text_string, output_path, template_path, video_pos=(100, 1150)):
    try:
        # Convert relative paths to absolute paths based on the project root
        ...
        # Load the template image and resize it to the target resolution
        ...
        # Define overlay size while preserving aspect ratio
        overlay_width = 920
        ...
        while True:
            ...
            # Resize the input frame to the overlay size
            overlay_video = cv2.resize(frame, (overlay_width, overlay_height))
            result = template_img.copy()

            # Overlay the video frame onto the template at the specified position
