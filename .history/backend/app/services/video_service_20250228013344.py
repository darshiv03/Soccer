...
def create_templated_video(input_video_path, text_string, video_pos, text_pos=(100, 1150)):
    try:
        # Ensure the generated_videos folder exists, or create it if it doesn't
        if not os.path.exists("generated_videos"):
            os.makedirs("generated_videos")  # Create the directory if it doesn't exist

        # Define the output video path within the generated_videos folder
        output_path = os.path.join("generated_videos", "generated_output_video.mp4")

        # Load the template image from the static folder
