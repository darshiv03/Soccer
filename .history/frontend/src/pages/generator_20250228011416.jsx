  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      const response = await axios.post(
        "http://127.0.0.1:8000/api/generate_video/",
        formData,
        {
          headers: {
            "Content-Type": "multipart/form-data",
          },
          responseType: "blob", // Receive the video as a blob
        }
      );

      if (response.status === 200) {
        // Create a URL for the video file from the blob
        const videoUrl = URL.createObjectURL(response.data); // Blob to URL
